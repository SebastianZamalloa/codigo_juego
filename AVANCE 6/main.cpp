...
int main()
{
	...
	RemoteControl* controlBoton1 = new RemoteControl;
	RemoteControl* controlBoton2 = new RemoteControl;
	controlBoton1->setCommand(botones[0]);
	controlBoton2->setCommand(botones[1]);
	...
	while (running)
	{
		...
		if (event.type == ALLEGRO_EVENT_DISPLAY_CLOSE)
			running = false;
		controlBoton1->recharBtn(event);
		controlBoton2->recharBtn(event);
		x = aliados.size();
		y = enemies.size();
		...
		if (event.type == ALLEGRO_EVENT_TIMER)
		{
			...
			controlBoton1->buttonPressed(2, aliados, x, true, prueba, false);
			controlBoton2->buttonPressed(1, enemies, y, false, prueba, false);
			for (int i = 0; i < x; i++)
			{
	...
	for (int i = 0; i < 2; i++)
		delete botones[i];
	delete controlBoton1, controlBoton2;
